...
import { phoneNumberValidator } from './input-phone.validator';
...
export class InputPhoneComponent implements ControlValueAccessor, OnInit, OnChanges {
  public innerMask = ''
  public innerMaskPrefix = ''
  // SAVE STATE
  public uuid: string = uuidv4()
  @Input() public fields: Field[] = [];

  @Input() value: InputPhoneValueTypes = '';
  ...
  @Input() selectedCountryISO: any = CountryISO.Russia;
  ...
  @Output() numberChange = new EventEmitter<ChangeData | null>()
  @Output() public modelChange: EventEmitter<InputPhoneValueWithId> = new EventEmitter<InputPhoneValueWithId>();
  ...
  constructor(private countryCodeData: CountryCode, private sanitization: DomSanitizer) {
  }
  ...
  public get selectedMask(): MaskI {
    return this.getAutoMaskObject
  }

  public get getAutoMaskObject(): MaskI {
    if (this.autoMask && this.selectedCountry.mask) {
      return this.selectedCountry.mask
    } else {
      return {mask: this.mask, maskPrefix: this.maskPrefix}
    }
  }
  ...
  */
  ...
  private formatValue(value: string): string {
    ...
    if (this.autoMask) {
      return value === this.innerMaskPrefix ? '' : cleanedValue
    } else {
      return cleanedValue.length === this.maskPrefix.length ? '' : cleanedValue
    }
  }
    ...
    if (this.autoMask) {
      ...
      setTimeout(() => {
        if (this.selectedCountry.mask) {
          this.innerMask = this.selectedCountry.mask.mask
          this.innerMaskPrefix = this.selectedCountry.mask.maskPrefix
        } else {
          this.innerMask = ''
          this.innerMaskPrefix = ''
        }
  ...
  public searchCountry() {
    if (!this.countrySearchText) {
      this.countryList.nativeElement
        .querySelector('.mrx-input-phone__country-list li')
        .scrollIntoView({
          behavior: 'smooth',
          block: 'nearest',
          inline: 'nearest',
  ...
  public onPhoneNumberChange(): void {
    ...
    if (!this.value || (this.autoMask && this.innerMaskPrefix === this.value)) {
      ...
      const emitValue = {
        ...
        unFormatNumber: this.formatValue(this.value)
      }
      this.numberChange.emit(emitValue)
      this.modelChange.emit({value: emitValue, id: this.uuid})
    }
  }
  ...
  protected getPhoneNumberMask(countryCode: string): MaskI {
    try {
      return MaskCode[countryCode]
    } catch (e) {
      // @ts-ignore
      return e;
    }
  ...
  protected fetchCountryData(): void {
    ...
    this.countryCodeData.allCountries.forEach((c) => {
      ...
      if (this.autoMask) {
        country.mask = this.getPhoneNumberMask(country.iso2.toUpperCase())
      }
